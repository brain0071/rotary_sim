...
class ROBOT:
    def __init__(self, mass, inertia, add_mass, quadratic_damp,  max_force_moment):
            print("======== 正在运行带执行器噪声的版本 (Noisy Actuator) ========")
            ...
            #lsz
            self.actuator_noise_std = np.array([0.2, 0.02, 0.02, 0.01, 0.01, 0.01])
    ...
    def update(self, dt, opt_u):
        ...
        #lsz
        noise_u = np.random.normal(0, self.actuator_noise_std)
        disturb_u = opt_u + noise_u
    
        # RK4 integration
        k1 = np.concatenate((self.p_dynamics(x), self.q_dynamics(x),
                            np.concatenate((self.v_dynamics(x, disturb_u), self.r_dynamics(x, disturb_u)))))

        # x_aux = x + (dt / 2) * k1
        x_aux = [x[i] + dt / 2 * k1[i] for i in range(13)]

        k2 = np.concatenate((self.p_dynamics(x_aux), self.q_dynamics(x_aux), 
                            np.concatenate((self.v_dynamics(x_aux, disturb_u), self.r_dynamics(x_aux, disturb_u)))))
